    ...
    try {
      ...
      if (error) {
        ...
        if (data.session) {
          ...
          // In case email confirmation is turned on in Supabase
    ...
    <div className="min-h-screen flex items-center justify-center bg-[#FAF7F2] p-4">
      <div className="bg-white rounded-2xl shadow-md border border-[#EDE8E0] p-8 w-full max-w-md">
        <div className="text-center mb-8">
          <h1 className="text-3xl font-bold font-display text-[#1C1410] mb-2">Join Click2Menu</h1>
          <p className="text-[#6B635A]">Create an account to start building menus</p>
        </div>
        ...
        <div className="relative my-6">
          ...
          <div className="relative flex justify-center text-sm">
            <span className="px-2 bg-white text-[#A09588]">Or register with email</span>
          </div>
        </div>

        <form onSubmit={handleRegister} className="space-y-4">
          <div>
            <label className="block text-sm font-medium text-[#1C1410] mb-1">Full Name</label>
            <div className="relative">
              <User className="absolute left-3 top-1/2 -translate-y-1/2 w-5 h-5 text-[#A09588]" />
              <input
                type="text"
                value={name}
